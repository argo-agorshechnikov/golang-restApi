...
func (r *ProductRep) CreateProductRep(product *models.Product) error {
	...
	_, err := r.db.Exec(query, product.ID, product.Name, product.Price, product.Description)
	return err
}
...
func (r *ProductRep) GetProductById(id string) (*models.Product, error) {
	...
	err := r.db.QueryRow(query, id).Scan(&product.ID, &product.Name, &product.Price, &product.Description)
	if err != nil {
		return nil, err
	}
